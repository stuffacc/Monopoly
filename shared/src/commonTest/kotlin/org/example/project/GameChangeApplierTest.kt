...
import org.example.project.data.repository.createField
...
class GameChangeApplierTest {
    ...
    @Test
    fun `Change Game Is Finished Game Change Test`() {
        val newGameState = GameChangeApplier.applyGameChanges(
            gameState = gameState,
            gameChanges = listOf<GameChange>(
                ChangeGameIsFinished(
                    isFinished = true
                )
            )
        )
        ...
        assertTrue {
            newGameState == (gameState.copy(isFinished = true))
        }
    }
